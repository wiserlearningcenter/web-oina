...
import {
  useCmsVoluntariadoAgenda,
  useCmsVoluntariadoSectionText,
} from "@/lib/cms/hooks";
...
export function VoluntariadoProximasActividades() {
  const edit = useVoluntariadoCmsEdit();
  const cmsItems = useCmsVoluntariadoAgenda();
  const sectionText = useCmsVoluntariadoSectionText();
  const fallback = getUpcomingAgendaItems(VOLUNTARIADO_PROXIMAS_ACTIVIDADES).map(
    (e) => ({
      ...e,
  ...
  if (edit?.ready) {
    const items = getUpcomingAgendaItems(edit.items.map(cmsEntryToAgenda)).map(
      (e) => ({
        ...e,
        image: resolveCmsMediaUrl(e.image) ?? e.image,
      }),
